...
| 世界模型预测观测（恒 0.5 兜底）| **预测价值变化 ΔV——✅ A 步骤已接线（f3e44de + 81cf190 + acc237b，价值头基类+叠加态多假设坍缩；dv 观测时点差值——review blocking 修复：原 body._prev 含已注入奖励→ΔV 被 baseline 吸收近似空转；security：grow 后 value_head 重置 + dv numel 校验；验证：world_loss 从恒 0.5 → 真实波动含奖励内容；收敛未证实——下阶段调优）** | Active Inference / §8 信号空转 |
