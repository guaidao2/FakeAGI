...
规则变化后 FakeAGI 长时窗（2000 tick）3/3 seed 恢复进食（52/62/55 次）
...
**口渴导航修复（欲望架构阶段 A——第二增量复活，b9b23dd）**：根因——`reflex_vote` 方向恒为食物（obs[0:2]），口渴仅触发条件；`get_action_bias` 无 thirst 偏置。修复（缺水朝水 obs[2:4] + thirst 偏置对称 hunger）后长时窗重跑：**FakeAGI 3/3 活满 2000 + 3/3 适应（52/62/55 次）vs pymdp 499 死 + 0 适应——"动态稳态 vs 静态偏好"以真机制成立**（接线审计 B1-B3 修复后复核——探索真实化不破坏长时窗结论；任务一阈值 0.95 修复后更稳）。

**接线审计（B1-B3）与任务一修复（短窗适应曲线）**：原 600 tick 适应 3/5（13-189）经审计发现部分依赖"伪 boredom"——数据真实化后 1/5（1）；任务一真实机制三修（阈值 0.95 利用方向信息/到达停留本能/扫掠无线索修正）后 2/5（27/8）——**适应增量如实表述为长时窗性质**（2000 tick 3/3 稳定 52-62），短窗受种子路径限制。
