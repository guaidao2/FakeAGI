...
**口渴导航修复（欲望架构阶段 A——第二增量复活，b9b23dd）**：根因——`reflex_vote` 方向恒为食物（obs[0:2]），口渴仅触发条件；`get_action_bias` 无 thirst 偏置。修复（缺水朝水 obs[2:4] + thirst 偏置对称 hunger）后长时窗重跑：**FakeAGI 3/3 活满 2000 + 3/3 适应（133-245 次）vs pymdp 499 死 + 0 适应——"动态稳态 vs 静态偏好"以真机制成立**。600 tick 主对比同步更新：适应 3/5（13-189，水优先轻微牺牲短时食物换长期存活）。
