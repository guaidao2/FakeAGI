...
**长时窗（2000 tick × 3seeds）**：
| 系统 | 存活 | 变化后食物 |
|------|------|-----------|
| FakeAGI | 3/3 活满 2000 | seed1: **397**（适应），seed0/2: 0 |
| pymdp | 3/3 死 499 | 0 |

**第二增量证据（长时窗暴露）**：pymdp 全部 499 tick 死亡 = **水耗尽**——其静态 C 偏好（固定水偏好）无法应对长期稳态需求；FakeAGI 的动态驱动力（缺水→去水）维持长期生存——**动态稳态 vs 静态偏好的增量**（公理② 活着 + 驱动力系统）。
