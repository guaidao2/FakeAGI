...
**代谢对称化修正（review 抓出——重要）**：原版 FakeAGI 端代谢 ~0.0005 vs pymdp 0.004（10× 不对称）——"动态稳态 vs 静态偏好"第二增量（原长时窗 FakeAGI 活满 2000 vs pymdp 499 死）**部分建立在此不对称上**。对称化后（FakeAGI 端也 -0.004/-0.002）：长时窗 FakeAGI 也死于稳态失衡（546-762——口渴驱动力不足）——第二增量撤销，降级为"存活略优"（546-762 vs 499）。**600 tick 主对比同步重跑：适应主张增强——FakeAGI 4/5 seed 恢复（70-270 次）vs pymdp 0/5（原慢代谢 2/5 为保守低估）。**
