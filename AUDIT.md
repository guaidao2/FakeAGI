...
- 长时窗第二增量（动态稳态）：3/3 保持（任务一后 52/62/55）——**探索真实化不破坏**——
  且机制更真（生理欲望→目标导航 + §3.4 水动力学）
- 经验：**接线验证必须用"数据流入断言"（monkeypatch 实测），不能只看调用点存在**

---

# 全量重跑复核（2026-08-06——主循环改动后 E1-E11）

**触发**：任务一 reflex 阈值 0.95 + 到达停留（主循环级改动）——按记忆纪律
（实验后自动验证文档论文）全量重跑 E1-E11。

**结果**：**全部通过**。
- E1：236 tick 适应（旧记录 28——数字更新）
- E2：OK
- E3：OK（改进 90.0%——旧记录 ~40%）
- E4：t=90/t=287 学会两步因果链（旧记录 t=8/t=20）
- **E5：WARN→OK——发现并修复断线**：suppress_reflex 只定义无消费点
  （"学习抑制反射"从未生效——GameNN 学会避险但 habit 0.25 压不过
  reflex 0.35）。接入 committee.decide reflex_scale 参数后转 OK。
- E6：OK（10 tick）
- E7-E11：全部 OK

**经验**：主循环级改动必须全量重跑——这次抓出 suppress_reflex 断线
（五查清单第 1 项"新信号是否有消费点"的反向：**旧信号是否仍有消费点**）。

> **终审修正（1db977d 后）**：上文 E1=236/E3=90.0%/E4=t=90/E5=OK 为
> 首轮跑值——panic 豁免修复后二轮落盘为 E1=57（OK）/E3=12.2%（WARN）/
> E4=t=303（OK）/E5=WARN（反射无威胁感知盲区）——单次跑数字波动大，
> 最终以 results_*_r2.txt 落盘与 PAPER/README 区间化记录为准（待任务②
> seed 均值化定稿）。
