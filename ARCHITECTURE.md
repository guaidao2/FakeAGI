...
- **⑥ 迁移价值评估**（`core/transfer_selector.py` + `test_transfer_selector.py`）：何时迁移的元认知——多假设贝叶斯 + 幅度加权（平局=迁移无优势→降）+ 保守阈值决策。同构经验升可靠性（0.63→选迁移）、异构经验降（0.56→选从头），有评估器 21.9 vs 无脑迁移 20.7（5 项全过）。
