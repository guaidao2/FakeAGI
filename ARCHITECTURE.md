...
- **概念迁移（路线 B）**（`test_concept_transfer.py` + `test_concept_scan.py`）：跨表面抽象"可消耗物"——训练 food+water 两种表面，零样本测 fruit 第三种表面——3 独立训练 seeds 全过（B 最差 47/50 vs 随机 14-20；**D 外观权重 0.257/0.224/0.245 全收敛<0.3**）；**结论升级"概念抽象（丢弃表面）"**（L2=0.05 扫描选定，预注册约束不变）；v1 教训（Q 表查表无法测概念/方向混叠）与降级→升级过程如实记录。
