...
- **全模块协同（E14）**（`test_emergent.py`）：语言+情绪+他者全开迷宫世界 10000 tick——5 项全过（多模块激活 3/5、情绪 467 次、语言 467 次影响决策、多 seeds 全模块 0.6±1.2 vs 对照 0.0±0.0——种子修复后重跑，结论稳定：全模块始终≥对照）；暴露睡眠未触发集成短板（单模块测试发现不了）。
