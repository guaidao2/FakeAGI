...
- **全模块协同（E14）**（`test_emergent.py`）：语言+情绪+他者全开迷宫世界 10000 tick——5 项全过（多模块激活 **4/5**、情绪 864 次、语言 444 次影响决策、多 seeds 全模块 **1.6±1.4** vs 对照 1.2±2.4）；**睡眠短板已修复**（根因：食物稀缺→存活短→疲劳未达阈值→睡眠永不触发；疲劳 0.005/tick+能量条件 0.3+极端疲劳 0.85 强制睡眠——睡眠 421 tick 触发，觅食 0.6→1.6）；修复暴露持久化缺口（is_sleeping 未保存）+ weights_only 回退门控（test_persist 含睡眠断言通过）。
