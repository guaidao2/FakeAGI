...
> + **护栏裁决**（a482958→6557aed——③ 动机选择器 vs 护栏字面）：
>   direct（直控）23.0/存活 1/10 vs hint（概念投票通道）19.3/存活
>   4/10——**hint 护栏内且存活 4 倍→默认可切 hint，direct 保留开关**
> + **pymdp 基线对比**（446cdd9——外部评审响应）：坐标系建立——
>   剥削 pymdp 优/适应 FakeAGI 部分占优（COMPARISON.md）+ **十一大
>   公理**（含⑪能量-时间-精度三角权衡、⑩存续拓扑学、④认识论子条款）
> + **概念符号化**（词-概念绑定——语言接地：词有「所指」、概念有
>   名字；听词→激活概念→预测价值→动作）+ **端到端验证**（听词→
>   行为价值 +20% 食物）+ **概念组合与自指**（可消耗物抽象 +
>   自我概念「我饿了」——概念层阶段 1-4 全部完成）
> + **语言生成**（概念→词 speak——双向语言）+ **社会智能四层优化**
...
> 数字复核（2026-08-06 全量重跑）：E1-E11 判定全部通过——主循环 reflex
> 改动（阈值 0.95/到达停留/suppress_reflex 接入）不破坏任何实验。

| 实验 | 结果 |
|------|------|
| E1 新奇适应 | ✅ 通过（8x8→16x16，236 tick 适应——2026-08 复核） |
| E2 多驱动力冲突 | ✅ 通过 |
| E3 空间记忆利用 | ✅ 通过（改进 90%——2026-08 复核） |
| E4 因果推理 | ✅ 通过（t=90 开关 → t=287 食物——2026-08 复核） |
| E5 反事实选择 | ✅ 通过（学会避险——2026-08 复核） |
| E6 规则变化适应 | ✅ 通过（水源移动后 10 tick 适应——2026-08 复核） |
...
> 零样本迁移成立（3 独立训练 seeds 最差 49/50）+ 外观权重收敛
