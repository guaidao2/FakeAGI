...
- [x] **E14 全模块协同**（`test_emergent.py`）— 语言+情绪+他者全开迷宫世界：5 项全过（多 seeds 觅食优势 1.6±1.4 vs 1.2±2.4）；**睡眠短板已修复**（根因链：食物稀缺→存活短→疲劳未达 0.7→睡眠永不触发；疲劳 0.005/tick+能量条件 0.3+极端疲劳强制睡眠——睡眠 421 tick 触发、觅食 0.6→1.6）；修复暴露持久化缺口（is_sleeping）+ weights_only 回退门控
