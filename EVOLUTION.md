...
> - ✅ **pymdp 基线对比（446cdd9——外部评审响应：坐标系建立）**：外部评审指出"预测误差=唯一信号是 active inference 的重新发现——需 pymdp 基线"。5x5 网格+规则变化（tick300 食物移位）同环境对比：**剥削 pymdp 优（变化前 295 次吃 vs FakeAGI ~110——精确贝叶斯推断）、适应 FakeAGI 占优（600 tick 3/5 seed 恢复 13-189 vs pymdp 0/5——多渠道误差修正 vs 单 A 矩阵 Dirichlet 更新；lr 0.1-1.0 扫描无改善）**；**长时窗第二增量（b9b23dd 口渴导航修复后）：FakeAGI 2000 tick 3/3 活满 + 3/3 适应（133-245）vs pymdp 499 死——动态稳态 vs 静态偏好（生理驱动耦合）**——差异定位：④=变分自由能确认（评审判断获实证）、精度加权/主动感知/前向模型 BPTT/剪枝引擎为真实增量（待实现）——COMPARISON.md 落盘
