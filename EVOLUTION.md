...
- [x] **路线 C 多智能体社会**（`test_social.py`）— 双 AGI 共享世界：他者模型接入修复（原缺失接口冲突=随机碰撞）+ **稀缺度-冲突曲线（诚实非单调 9/29/29 + 丰富 55/17/14/2）**——稀缺-难获 29 最高（假说前半支持）；**假说后半补验（预注册判据）：有效性 FAIL——成本>0 双双饿死，"共存"无法检验（非假说为假）**；**目标坚持机制尝试（生物洞察"坚持=未完成/放弃=预测误差"）：AGI 内部 override 实现 + 4 通路单元验证过，但全量无效——外部注入不能替代内部学习（AGI 未学会停留=工作，需进度观测+shaping），旁路假象（食物 88）已废弃**；ablation（限稀缺-难获）他者开 29 vs 关 14——社会感知驱动冲突约翻倍；seed 修复教训如实记录
