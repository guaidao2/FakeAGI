...
> - ✅ **护栏裁决实验（a482958——③ 动机选择器 vs 护栏字面）**：概念驱动（匹配→action=0）是"价值直驱动作"与护栏"价值只喂学习系统不直接驱动动作"的张力点——n=10 双模式对照：**direct（直控）食物 23.0±6.4 存活 1/10 vs hint（概念投票通道——committee concept 票权重 0.20/恐慌归零）19.3±7.3 存活 4/10**——**效率-稳健权衡：hint 护栏内且存活 4 倍（不锁定行为——价值经决策层仲裁），direct 食物略高**——裁决：**默认可切 hint（公理内实现），direct 保留开关**；修复链：hint 初版 grow 冲突（LNN 生长也 grow GameNN→双 grow 维度崩 72/73/74）→ 投票通道零维度方案 + gamenn.learn 维度防护（防御性）
