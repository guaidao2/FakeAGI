...
- [x] **路线 C 多智能体社会**（`test_social.py`）— 双 AGI 共享世界：**核心发现**——他者模型接入修复（原缺失接口冲突=随机碰撞）+ **稀缺度-冲突曲线成立**（稀缺-难获 29 > 稀缺-易得 9 > 中等 5 > 丰富-难获 2——用户假设验证）+ **ablation 归因**（他者开 29 vs 关 14——社会感知驱动冲突翻倍）；生存优势仍未见；seed 修复教训（缺 torch.manual_seed 初版伪通过）如实记录
