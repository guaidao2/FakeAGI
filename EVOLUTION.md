...
- [x] **路线 B 概念迁移**（`test_concept_transfer.py`）— 跨表面抽象"可消耗物"：见过两种表面→第三种表面零样本 100% 迁移（3 独立 seeds 全 50/50 vs 随机 19-23）；结论诚实降级"跨表面泛化"（外观权重 0.358 未收敛）；v1 教训（Q 表查表无法测概念）如实记录
