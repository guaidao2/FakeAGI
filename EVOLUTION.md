...
- [x] **路线 C 多智能体社会**（`test_social.py`）— 双 AGI 共享世界：他者模型接入修复（原缺失接口冲突=随机碰撞）+ **稀缺度-冲突曲线（诚实非单调 9/29/29 + 丰富 55/17/14/2）**——稀缺-难获 29 最高（假说前半支持）；**假说后半补验（预注册判据）：有效性 FAIL——成本>0 双双饿死，"共存"无法检验（非假说为假）**；**目标坚持机制尝试（生物洞察"坚持=未完成/放弃=预测误差"）：AGI 内部 override 实现 + 4 通路单元验证过；接线修复（None 哨兵）后**工作能力解锁成功**（停留代谢减半+放弃冷却：有效性 OK，seed42 成本 60 活满 3000 完成 5 次工作）；稀缺配置=到达率限制（坚持 0）；共存判据仍 FAIL（min 存活不降 FAIL+冲突率无降幅）——"丰富难得→共存"未涌现，需新机制（如轮流信号）**；ablation 反转（他者开 20 vs 关 22——参数变化后无显著影响）；seed 修复教训如实记录
