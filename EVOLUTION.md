...
- [x] **路线 C 多智能体社会**（`test_social.py`）— 双 AGI 共享世界：他者模型接入修复（原缺失接口冲突=随机碰撞）+ **稀缺度-冲突曲线（诚实非单调 9/29/29 + 丰富 55/17/14/2）**——稀缺-难获 29 最高（假说前半支持）；**假说后半补验（预注册判据）：有效性 FAIL——成本>0 双双饿死，"共存"无法检验（非假说为假）**；**目标坚持机制尝试（生物洞察"坚持=未完成/放弃=预测误差"）：AGI 内部 override 实现 + 4 通路单元验证过；接线修复后（None 哨兵——初版 last_action=0 误判 bug 数据作废）重测：丰富冲突单调降 95% 现象出现 + 取食 8/2，但共存判据仍 FAIL（min 存活 629，工作能耗耗尽）——现象部分支持、生存未达标，待能耗平衡优化**；ablation（限稀缺-难获）他者开 29 vs 关 14——社会感知驱动冲突约翻倍；seed 修复教训如实记录
