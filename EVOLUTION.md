...
- [x] **路线 C 多智能体社会**（`test_social.py`）— 双 AGI 共享世界：他者模型接入修复（原缺失接口冲突=随机碰撞）+ **稀缺度-冲突曲线（诚实非单调 9/29/5/17）**——稀缺-难获 29 最高（假设前半支持）、丰富-难获 17 反弹（后半未证，方差极端 seed 单点驱动）；**ablation（限稀缺-难获）他者开 29 vs 关 14**——社会感知驱动冲突约翻倍；生存优势未见；seed 修复教训如实记录
